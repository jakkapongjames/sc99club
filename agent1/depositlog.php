    ...
    <div id="wrapper">
       ...
       <?php include'nav.php'?>
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-6">
                    <h1 class="page-header">รายการฝาก</h1>
                </div>
...
<?php
        ...
        $sql = "SELECT category,transactionid,user,agent,date_now,surelastname,banknumber,bank,tel,idline,deposit,bonus,withdraw,tperson,aperson,note,liststatus,ip
                FROM scclub_data WHERE (liststatus = 'confirm' AND withdraw ='')
                ORDER BY transactionid DESC LIMIT 3000"; // ASC เรียงน้อยไปมาก หรือ DESC เรียง มากไปน้อย

        if(!empty($_POST['date_from']) && !empty($_POST['date_to'])){
            $date_from = date('Y-m-d',strtotime($_POST['date_from']));
            $date_to = date('Y-m-d',strtotime($_POST['date_to']));
            $sql = "SELECT category,transactionid,user,agent,date_now,surelastname,banknumber,bank,tel,idline,deposit,bonus,withdraw,tperson,aperson,note,liststatus,ip
                    FROM scclub_data WHERE (liststatus = 'confirm' AND date_now BETWEEN '$date_from' AND '$date_to' AND withdraw ='')
    ...
    $('#dataTables-example').DataTable( {
        ...
        "fnRowCallback": function ( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
            if ( aData[1] == "เติมเงิน" )
                    ...
                    else if ( aData[1] == "สมัครสมาชิก" )
                    {
                        $('td', nRow).css('background-color', '#6cb6ea');
                    }
