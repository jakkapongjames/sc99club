...
<?php
        ...
        $sql = "SELECT category,transactionid,user,date_now,surelastname,banknumber,bank,tel,idline,deposit,withdraw,bonus,withdraw,tperson,aperson,note,liststatus,ip
                FROM scclub_data WHERE (liststatus = 'confirm')
        ...
        if(!empty($_POST['date_from']) && !empty($_POST['date_to'])){
            ...
            $sql = "SELECT category,transactionid,user,date_now,surelastname,banknumber,bank,tel,idline,deposit,withdraw,bonus,withdraw,tperson,aperson,note,liststatus,ip
                    FROM scclub_data WHERE (liststatus = 'confirm' AND date_now BETWEEN '$date_from' AND '$date_to')
...
$(document).ready(function() {
    $('#dataTables-example').DataTable( {
        ...
        "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
                    if ( aData[1] == "สมัครสมาชิก" )
                    {
                        $('td', nRow).css('background-color', '#6cb6ea');
                    }
                    else if ( aData[1] == "เติมเงิน" )
                    {
                        $('td', nRow).css('background-color', '#5cb85c');
                    }
                    else if ( aData[1] == "ถอนเงิน" )
                    {
                        $('td', nRow).css('background-color', '#f66');
                    }
            ...
            var Totalall = 0;
            Totalall = parseInt(pageTotal)-parseInt(pageTotal2);
