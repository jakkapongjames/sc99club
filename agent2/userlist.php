    ...
    <div id="wrapper">
       ...
       <?php include'nav.php'?>
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-6">
                    <h1 class="page-header">รายชื่อสมาชิก</h1>
                </div>
...
<?php
        ...
        $sql = "SELECT transactionid,category,user,agent,date_now,surelastname,banknumber,bank,tel,idline,deposit,bonus,withdraw,tperson,aperson,note,liststatus,ip,avatar,smsname
                FROM scclub_data2 WHERE category='สมัครสมาชิก'
                Group BY user
                HAVING liststatus='confirm'
                ORDER BY transactionid DESC";
                ; // ASC เรียงน้อยไปมาก หรือ DESC เรียง มากไปน้อย

        if(!empty($_POST['date_from']) && !empty($_POST['date_to'])){
            $date_from = date('Y-m-d',strtotime($_POST['date_from']));
            $date_to = date('Y-m-d',strtotime($_POST['date_to']));
            $sql = "SELECT transactionid,category,user,agent,date_now,surelastname,banknumber,bank,tel,idline,deposit,bonus,withdraw,tperson,aperson,note,liststatus,ip,avatar,smsname
                    FROM scclub_data2 WHERE (category='สมัครสมาชิก' AND date_now BETWEEN '$date_from' AND '$date_to' AND withdraw ='')
                Group BY user
                HAVING liststatus='confirm'
                ORDER BY transactionid DESC";
        }
        ...
        while($row = $query->fetch()) {
            ...
            echo '
                                    ...
                                    <tr>
                                        <td>
                                        <center>
                                        <img src="'.$row['avatar'].'" style="width: 50px; height: 50px; border-radius: 50%;">
                                        </center>
                                        </td>
                                        <td>'.$row['user'].'</td>
                                        ...
                                        <td>'.$row['banknumber'].'<br>'.$row['bank'].'</td>
                                        <td>'.$row['smsname'].'</td>
                                        <td>'.$row['tel'].'</td>
                                        <td>'.$row['idline'].'</td>
                                        <td>'.$row['deposit'].'</td>
                                        <td>'.$row['aperson'].'</td>
                                        <td>
                                        <a href="edituser.php?transactionid='.$row['transactionid'].'">แก้ไข</a>
                                        </td>
                                    </tr>'
                                     ;
        }
