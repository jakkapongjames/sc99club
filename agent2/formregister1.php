...
if($name !=''||$deposit !=''){
//Insert Query of SQL
$query = mysql_query("insert into scclub_data2(category, user,agent, date_now, surelastname, bank, banknumber, tel, idline, deposit, bonus, tperson, invite, avatar, smsname) values ('$category', '$user', '$agent', '$date_now', '$surelastname', '$bank', '$banknumber', '$tel', '$idline', '$deposit', '$bonus', '$tperson' ,'$invite','$avatar' ,'$smsname')");
...
$strSQL = "SELECT * FROM scclub_data2 WHERE category='สมัครสมาชิก' AND user = '".$_GET["sendget"]."' or smsname = '".$_GET["sendget"]."' ";
if(empty($_GET)) {
            $strSQL = "SELECT * FROM scclub_data2 WHERE smsname or user ='SCCLUB99' ";
        }
